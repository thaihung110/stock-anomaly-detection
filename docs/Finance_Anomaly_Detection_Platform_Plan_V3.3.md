...
5. **News**: **chỉ dùng Finnhub `/company-news` REST** làm nguồn duy nhất — poll mỗi 5 phút, mỗi symbol 1 request, dedup bằng MD5(url) in-memory, đẩy vào Kafka `raw.stock.news`, Spark ghi vào Iceberg `bronze.raw_news_articles`.
6. **Company metadata**: Finnhub `/stock/profile2` + `/stock/metric` batch weekly → Iceberg `bronze.raw_company_info` → Gold `dim_symbol`. (yfinance `.info` requires undocumented cookie/crumb auth and is unreliable — replaced by Finnhub documented REST API.)
7. **Không dùng TimescaleDB** — toàn bộ lưu trữ dựa trên Kafka (streaming) và Iceberg/MinIO (batch + analytics).

```
Bronze Source Map (3 nguồn, không overlap):

  daily OHLCV    →  yfinance v8/chart batch           →  bronze.raw_ohlcv_daily   [Iceberg, incremental daily]
  company meta   →  Finnhub /stock/profile2 + /metric →  bronze.raw_company_info  [Iceberg, weekly]
  ...
  news           →  Finnhub /company-news REST (5min, per symbol)  →  bronze.raw_news_articles [Iceberg, via Spark]
```

---

### 2.1 Nguồn 1 — Yahoo Finance (yfinance) — MAIN OHLCV + Quotes
...
> **Lưu ý:** `yfinance.Ticker.info` (company metadata) **không còn được dùng** trong stack này. Yahoo Finance `v10/quoteSummary` yêu cầu cookie + crumb auth (undocumented internal API, trả về 401 khi không có session cookie). Company metadata được lấy từ **Finnhub `/stock/profile2` + `/stock/metric`** (xem Nguồn 2) — documented REST API, chỉ cần API key.
...
**Schema: `bronze.raw_company_info`** — Company metadata từ **Finnhub `/stock/profile2` + `/stock/metric`**, Iceberg

> **Nguồn dữ liệu:** Finnhub REST API (documented, chỉ cần API key — không cần cookie/crumb):
> - `/stock/profile2?symbol={s}&token={key}` → name, exchange, country, currency, website, marketCapitalization (×1M), shareOutstanding (×1M), finnhubIndustry
> - `/stock/metric?symbol={s}&metric=all&token={key}` → beta, peBasicExclExtraTTM, 52WeekHigh/Low, 50DayMA, 200DayMA, dividendYieldIndicatedAnnual
>
> **Các field không có trên Finnhub free tier:** `quote_type`, `sector` (riêng biệt), `forward_pe` → lưu `NULL`.

```sql
CREATE TABLE bronze.raw_company_info (
    symbol              VARCHAR(20)   PRIMARY KEY,
    short_name          VARCHAR(200),                -- Finnhub profile2.name
    long_name           VARCHAR(200),                -- Finnhub profile2.name (same field)
    exchange            VARCHAR(20),                 -- Finnhub profile2.exchange
    quote_type          VARCHAR(20),                 -- NULL (not available on Finnhub free tier)
    sector              VARCHAR(100),                -- NULL (Finnhub has industry only via finnhubIndustry)
    industry            VARCHAR(200),                -- Finnhub profile2.finnhubIndustry
    country             VARCHAR(50),                 -- Finnhub profile2.country
    currency            VARCHAR(5),                  -- Finnhub profile2.currency
    website             TEXT,                        -- Finnhub profile2.weburl
    market_cap          BIGINT,                      -- Finnhub profile2.marketCapitalization × 1,000,000
    beta                DOUBLE,                      -- Finnhub metric.beta
    trailing_pe         DOUBLE,                      -- Finnhub metric.peBasicExclExtraTTM
    forward_pe          DOUBLE,                      -- NULL (not available on Finnhub free tier)
    fifty_two_week_high DOUBLE,                      -- Finnhub metric.52WeekHigh
    fifty_two_week_low  DOUBLE,                      -- Finnhub metric.52WeekLow
    fifty_day_average   DOUBLE,                      -- Finnhub metric.50DayMA
    two_hundred_day_avg DOUBLE,                      -- Finnhub metric.200DayMA
    shares_outstanding  BIGINT,                      -- Finnhub profile2.shareOutstanding × 1,000,000
    dividend_yield      DOUBLE,                      -- Finnhub metric.dividendYieldIndicatedAnnual
...
### 2.2 Nguồn 2 — Finnhub REST — Company Metadata (`/stock/profile2` + `/stock/metric`)

**API:** `GET https://finnhub.io/api/v1/stock/profile2?symbol=X&token=KEY` và `GET https://finnhub.io/api/v1/stock/metric?symbol=X&metric=all&token=KEY`

**Rate limit (free):** 60 req/min — 500 symbols × 2 endpoints × weekly batch ≈ 1,000 requests / 7 ngày ≈ safe

**Tại sao chọn Finnhub thay vì yfinance `.info`:**

- **Documented public API** — không cần cookie/crumb session (Yahoo `v10/quoteSummary` yêu cầu undocumented auth, dễ bị break).
- **Cùng API key** đã dùng cho Finnhub WebSocket trades và `/company-news` — không cần quản lý thêm credential.
- **Data từ `/stock/profile2`:** name, exchange, country, currency, weburl, marketCapitalization (×1M), shareOutstanding (×1M), finnhubIndustry.
- **Data từ `/stock/metric`:** beta, peBasicExclExtraTTM (trailing PE), 52WeekHigh/Low, 50DayMA, 200DayMA, dividendYieldIndicatedAnnual.

**Volume ước tính:**

- 500 symbols × weekly fetch ≈ **~5 MB** (JSON, small footprint)

**Ingest strategy:**

- Spark batch job `company-info-loader` (Scala, weekly): fetch profile2 + metric cho mỗi symbol → MERGE INTO `bronze.raw_company_info` (upsert by symbol)
- Rate-limit friendly: sequential requests với 200ms delay giữa các symbol

---

### 2.3 Nguồn 3 — Finnhub — MAIN Tick Data (chỉ WebSocket trades)

**API:** WebSocket `wss://ws.finnhub.io?token=<API_KEY>`[^15][^16]

> **Lưu ý:** Finnhub được dùng cho **Company Info REST** (Nguồn 2), **WebSocket trade tick** (Nguồn 3), và **REST `/company-news`** (Nguồn 4). Cùng 1 API key, cùng free-tier 60 req/min — company info batch chạy weekly nên không ảnh hưởng quota realtime.
...
### 2.4 Nguồn 4 — Finnhub `/company-news` REST — Nguồn news duy nhất

**API:** `GET https://finnhub.io/api/v1/company-news?symbol=X&from=YYYY-MM-DD&to=YYYY-MM-DD&token=KEY`

**Rate limit (free):** 60 req/min — 50 symbols × 1 req/poll × poll mỗi 5 phút = **10 req/min**, an toàn 6× dưới giới hạn.

**Tại sao chọn Finnhub `/company-news` thay vì NewsAPI:**

- **Cùng API key** đã dùng cho Finnhub WebSocket trades — không cần quản lý thêm credential.
- **Symbol-native**: mỗi request trả về news gắn với symbol cụ thể, không có keyword noise như NewsAPI `/everything?q=NVDA`.
- **Lookback linh hoạt**: có thể query bất kỳ khoảng ngày nào (không bị giới hạn 24h của NewsAPI free tier).
- **Dedup đơn giản**: in-memory MD5(url) set, max 10,000 entries — không cần DB phụ.
- **Rate limit dư dả**: 50 symbols × 1.1s delay giữa các symbol = ~45s/cycle, 10 req/min, thoải mái trong 60 req/min.

**Poll strategy:**

| Tham số             | Giá trị        | Lý do                                                       |
| ------------------- | -------------- | ----------------------------------------------------------- |
| `poll_interval_sec` | 300s (5 phút)  | Cân bằng freshness và rate limit                            |
| `lookback_days`     | 2 ngày         | Window rộng đủ để catch bài cũ + dedup xử lý overlap        |
| `request_delay_sec` | 1.1s/symbol    | 50 symbols × 1.1s ≈ 55s/cycle, dưới 60 req/min              |
| `dedup_max_size`    | 10,000 entries | In-memory MD5 set; reset khi đầy (stateless across restart) |

**Volume ước tính:** 50 symbols × 5 articles/poll × 288 polls/ngày = ~72,000 articles/ngày (trước dedup); sau dedup thực tế ~500–1,000 bài mới/ngày ≈ **~200 MB/năm**
...
CREATE TABLE bronze.raw_news_articles (
    ...
    symbol          VARCHAR(20)   NOT NULL,   -- symbol từ Finnhub /company-news
    headline        TEXT          NOT NULL,   -- Finnhub "headline"
    summary         TEXT,                     -- Finnhub "summary" (nullable)
    url             TEXT          NOT NULL,
    source          VARCHAR(100),             -- Finnhub "source"
    category        VARCHAR(50),              -- Finnhub "category" (nullable)
    published_at_ms BIGINT        NOT NULL,   -- epoch ms (Finnhub "datetime" × 1000)
    fetched_at_ms   BIGINT        NOT NULL,   -- epoch ms khi producer fetch
    PRIMARY KEY (article_id)
) USING iceberg PARTITIONED BY (days(CAST(from_unixtime(published_at_ms / 1000) AS DATE)));
```

**Ingest flow:**

```
finnhub-news-producer (Python async, poll every 5min per symbol)
  └─► Dedup: MD5(url) in-memory set (max 10k)
        └─► Kafka: raw.stock.news  (key = symbol bytes)
              └─► Spark Structured Streaming (news-ingest-stream)
...
SELECT headline, summary, published_at_ms
FROM bronze.raw_news_articles
WHERE symbol = 'NVDA'
  AND published_at_ms >= (UNIX_TIMESTAMP() - 6 * 3600) * 1000
...
### 2.5 Tổng hợp Bronze Layer — 3 nguồn, không overlap, không TimescaleDB

| Loại dữ liệu     | Nguồn                                        | Lưu trữ                                 | Tần suất          | Đến đâu?                                |
| ---------------- | -------------------------------------------- | --------------------------------------- | ----------------- | --------------------------------------- |
| Daily OHLCV      | yfinance `v8/chart` batch                    | **Iceberg**: `bronze.raw_ohlcv_daily`   | Daily 06:00 UTC   | → `silver.ohlcv_daily`                  |
| Company metadata | Finnhub `/stock/profile2` + `/stock/metric`  | **Iceberg**: `bronze.raw_company_info`  | Weekly            | → `gold.dim_symbol`                     |
...
| News articles    | Finnhub `/company-news` REST                 | **Iceberg**: `bronze.raw_news_articles` | Every 5min/symbol | → LLM Agent + `silver.news_clean`       |
...
| Finnhub company info (500 symbols, `/profile2` + `/metric`)      | ~5 MB           | Free[^16] |
| Finnhub tick data (Kafka, 2 năm, aggregate → silver.ohlcv_1min)  | ~2.1 GB         | Free[^16] |
| Finnhub company-news articles (2 năm, sau dedup)                 | ~200 MB         | Free[^16] |
...
│  yfinance WS  │  Finnhub WS  │  yfinance batch  │                 │
...
│  raw.stock.news     ← Finnhub /company-news polling (5min/symbol)  │
...
CREATE TABLE silver.news_clean (
    ...
    url             TEXT,
    data_source     VARCHAR(20),              -- "finnhub"
...
- Query Iceberg `bronze.raw_news_articles` (Trino) cho symbol trong 6h gần nhất
...
│                                                                        │
│  [yfinance WebSocket]      [Finnhub WebSocket]    [Finnhub /company-news]│
│  quote: AAPL $178.52       trades: AAPL 100sh     articles (5min/sym) │
│       │                         │                      │               │
│       ▼                         ▼                      ▼               │
...
│  - (Nguồn: Finnhub /company-news → Iceberg) (prev_close, bb_upper/low)│
...
| `raw.stock.news`   | Finnhub news producer  | Spark writer → `bronze.raw_news_articles` (Iceberg) | JSON (NewsArticle)    | News feed duy nhất (Finnhub /company-news, per symbol)        |
...
| LLM Agent — news fetch           | Trino query `bronze.raw_news_articles` | ~100–500ms       |
...
│  │    Producer: finnhub-news-producer (company-news, 5 min/sym)  │   │
│  ├──────────────────────────────────────────────────────────────┤   │
...
| Finnhub                           | $0.00   | Free 60/min — WS trades + /company-news[^16] |
...
- Finnhub /company-news polling (finnhub-news-producer) → raw.stock.news → bronze.raw_news_articles
