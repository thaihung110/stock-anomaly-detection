...
object CatalogConfigurator {
  ...
  def ensureTableExists(spark: SparkSession): Unit = {
    ...
    try {
      spark.sql("""
      CREATE TABLE IF NOT EXISTS gravitino_bronze.raw.raw_company_info (
    ...
    """)
      ...
      case e: Exception =>
        throw new RuntimeException(s"Failed to create table gravitino_bronze.bronze.raw_company_info: ${e.getMessage}", e)
    }
  }
}
