...
object AppConfig {
  def fromEnv(): AppConfig = AppConfig(
    ...
    inputTable                 = sys.env.getOrElse("INPUT_TABLE", "gravitino_gold.gold.fact_ohlcv_daily"),
    dimSymbolTable             = sys.env.getOrElse("DIM_SYMBOL_TABLE", "gravitino_gold.gold.dim_symbol"),
    outputTable                = sys.env.getOrElse("OUTPUT_TABLE", "gravitino_gold.gold.rule_engine_context"),
