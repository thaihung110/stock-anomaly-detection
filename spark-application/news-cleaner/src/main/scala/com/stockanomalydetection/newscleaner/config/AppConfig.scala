...
object AppConfig {
  def fromEnv(): AppConfig = AppConfig(
    ...
    inputTable                 = sys.env.getOrElse("INPUT_TABLE", "gravitino_bronze.raw.raw_news_articles"),
    outputTable                = sys.env.getOrElse("OUTPUT_TABLE", "gravitino_silver.normalized.news_clean")
  )
}
