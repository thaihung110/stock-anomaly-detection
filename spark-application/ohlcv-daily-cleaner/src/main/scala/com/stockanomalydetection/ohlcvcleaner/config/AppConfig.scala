...
object AppConfig {
  def fromEnv(): AppConfig = AppConfig(
    ...
    inputTable                 = sys.env.getOrElse("INPUT_TABLE", "gravitino_catalog.raw.raw_ohlcv_daily"),
    outputTable                = sys.env.getOrElse("OUTPUT_TABLE", "gravitino_catalog.normalized.ohlcv_daily")
  )
}
