...
object CatalogConfigurator {
  ...
  def ensureTableExists(spark: SparkSession): Unit = {
    ...
    try {
      spark.sql("""
      CREATE TABLE IF NOT EXISTS gravitino_catalog.raw.raw_ohlcv_daily (
    ...
    """)
      ...
      case e: Exception =>
        throw new RuntimeException(s"Failed to create table gravitino_catalog.raw.raw_ohlcv_daily: ${e.getMessage}", e)
    }
  }
}
