...
object BatchProgressListener extends StreamingQueryListener {
  ...
  override def onQueryStarted(event: QueryStartedEvent): Unit =
    log.info(s"[news-ingest-stream] Query started: id=${event.id} name=${event.name}")
  ...
  override def onQueryProgress(event: QueryProgressEvent): Unit = {
    ...
    log.info(
      s"[news-ingest-stream] Batch ${p.batchId}: " +
  ...
  override def onQueryTerminated(event: QueryTerminatedEvent): Unit =
    event.exception match {
      case Some(err) => log.error(s"[news-ingest-stream] Query terminated with error: $err")
      case None      => log.info(s"[news-ingest-stream] Query terminated cleanly: id=${event.id}")
    }
}
