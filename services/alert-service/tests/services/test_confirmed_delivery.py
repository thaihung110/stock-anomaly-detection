...
class TestFanOutConfirmed:
    @pytest.mark.asyncio
    async def test_confirmed_uses_html_parse_mode(self) -> None:
        telegram = AsyncMock()
        sub = Subscriber(user_id=uuid4(), chat_id=1001)
        delivery = AlertDeliveryService(telegram, _cache([sub]), _cfg(), _history_writer())
    ...
    async def test_confirmed_still_writes_history(self) -> None:
        ...
        delivery = AlertDeliveryService(telegram, _cache([sub]), _cfg(), history_writer)
...
class TestDeliverFollowup:
    ...
    async def test_followup_fans_out_to_subscribers(self) -> None:
        ...
        delivery = AlertDeliveryService(telegram, _cache(subs), _cfg(), _history_writer())
    ...
    @pytest.mark.asyncio
    async def test_followup_uses_html_parse_mode(self) -> None:
        telegram = AsyncMock()
        sub = Subscriber(user_id=uuid4(), chat_id=1001)
        delivery = AlertDeliveryService(telegram, _cache([sub]), _cfg(), _history_writer())
    ...
    async def test_followup_no_recipients_is_silent(self) -> None:
        telegram = AsyncMock()
        delivery = AlertDeliveryService(telegram, _cache([]), _cfg(), _history_writer())
