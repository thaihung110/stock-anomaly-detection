...
import pytest
from alert_service.infrastructure.telegram_client import TelegramError, TelegramPermanentError, TelegramRateLimitError

from alert_service.core.config import Settings
from alert_service.services.delivery import AlertDeliveryService
from alert_service.services.formatter import format_custom_message
...
class TestDeliverCustomRouting:
    ...
    async def test_sends_to_event_chat_id_when_routing_enabled(self) -> None:
        ...
        delivery = AlertDeliveryService(telegram, cache, cfg, AsyncMock())
...
class TestDeliverCustomRateLimiter:
    ...
    async def test_rate_limiter_called_before_send(self) -> None:
        ...
        cache = AsyncMock()
        cfg = _cfg()
        delivery = AlertDeliveryService(telegram, cache, cfg, AsyncMock(), rate_limiter=rate_limiter)
    ...
    async def test_no_rate_limiter_send_still_works(self) -> None:
        ...
        cache = AsyncMock()
        cfg = _cfg()
        delivery = AlertDeliveryService(telegram, cache, cfg, AsyncMock(), rate_limiter=None)
