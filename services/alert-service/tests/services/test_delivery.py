...
def _history_writer() -> AsyncMock:
    """Fake ``HistoryWriter`` — ``append_batch`` succeeds by default."""
    return AsyncMock()
...
async def test_fans_out_to_every_subscriber() -> None:
    ...
    cache = _cache(subs)
    cfg = _cfg()
    history_writer = _history_writer()

    delivery = AlertDeliveryService(telegram, cache, cfg, history_writer)
    await delivery.fan_out(_event())

    # History written ONCE (batch) before Telegram sends, containing both user_ids.
    history_writer.append_batch.assert_awaited_once()
    written_ids = set(history_writer.append_batch.await_args.kwargs["user_ids"])
...
async def test_no_subscribers_drops_alert() -> None:
    ...
    history_writer.append_batch.assert_not_awaited()
...
async def test_history_write_failure_aborts_fan_out() -> None:
    ...
    history_writer.append_batch.side_effect = RuntimeError("iceberg down")
    delivery = AlertDeliveryService(telegram, cache, cfg, history_writer, dlq=dlq)
    await delivery.fan_out(_event())
...
async def test_history_timeout_aborts_fan_out_without_dlq() -> None:
    ...
    history_writer = AsyncMock()
    history_writer.append_batch.side_effect = TimeoutError()
    delivery = AlertDeliveryService(telegram, cache, cfg, history_writer, dlq=dlq)
    await delivery.fan_out(_event())
...
async def test_rate_limiter_acquired_before_send() -> None:
    ...
    delivery = AlertDeliveryService(telegram, cache, cfg, history_writer, rate_limiter=limiter)
    await delivery.fan_out(_event())
...
# ── Phase 4 — deliver_admin_only (fan-out disabled) ───────────────────────
# Previously ``main._deliver_admin`` — moved onto AlertDeliveryService so it
# shares _write_history_or_abort / _send_and_dlq with fan_out.


@pytest.mark.asyncio
async def test_admin_only_writes_history_then_sends() -> None:
    telegram = AsyncMock()
    cfg = _cfg(admin_chat="ADMIN_CHAT")
    history_writer = _history_writer()

    delivery = AlertDeliveryService(telegram, None, cfg, history_writer)
    await delivery.deliver_admin_only(_event(), "hello", parse_mode="Markdown")

    history_writer.append_batch.assert_awaited_once()
    assert history_writer.append_batch.await_args.kwargs["user_ids"] == [None]
    telegram.send_message.assert_awaited_once_with(
        "ADMIN_CHAT", "hello", parse_mode="Markdown"
    )


@pytest.mark.asyncio
async def test_admin_only_history_failure_aborts_and_dlqs_with_admin_chat() -> None:
    telegram = AsyncMock()
    cfg = _cfg(admin_chat="ADMIN_CHAT")
    dlq = AsyncMock()
    history_writer = AsyncMock()
    ...
    delivery = AlertDeliveryService(telegram, None, cfg, history_writer, dlq=dlq)
    await delivery.deliver_admin_only(_event(), "hello", parse_mode="Markdown")
...
@pytest.mark.asyncio
async def test_admin_only_history_timeout_aborts_without_dlq() -> None:
    telegram = AsyncMock()
    cfg = _cfg(admin_chat="ADMIN_CHAT")
    dlq = AsyncMock()
    history_writer = AsyncMock()
...
@pytest.mark.asyncio
async def test_admin_only_telegram_failure_is_classified_and_dlqd() -> None:
    telegram = AsyncMock()
    telegram.send_message.side_effect = TelegramPermanentError("bad chat")
    cfg = _cfg(admin_chat="ADMIN_CHAT")
    dlq = AsyncMock()
    history_writer = _history_writer()
...
@pytest.mark.asyncio
async def test_admin_only_rate_limiter_acquired_before_send() -> None:
    telegram = AsyncMock()
    cfg = _cfg(admin_chat="ADMIN_CHAT")
    limiter = AsyncMock()
    history_writer = _history_writer()

    delivery = AlertDeliveryService(telegram, None, cfg, history_writer, rate_limiter=limiter)
    await delivery.deliver_admin_only(_event(), "hello", parse_mode="Markdown")

    limiter.acquire.assert_awaited_once_with("ADMIN_CHAT")


# ── _require_cache guard (survives python -O, unlike a bare assert) ──────


@pytest.mark.asyncio
async def test_fan_out_without_cache_raises_runtime_error() -> None:
    telegram = AsyncMock()
    cfg = _cfg()
    history_writer = _history_writer()
    ...
    with pytest.raises(RuntimeError, match="fan_out requires a cache"):
        await delivery.fan_out(_event())


@pytest.mark.asyncio
async def test_deliver_followup_without_cache_raises_runtime_error() -> None:
    telegram = AsyncMock()
    cfg = _cfg()
    history_writer = _history_writer()
    ...
    with pytest.raises(RuntimeError, match="deliver_followup requires a cache"):
        await delivery.deliver_followup(
            FollowUpEvent(
                ref_alert_id="alert-1",
                symbol="AAPL",
